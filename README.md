...
スケジュールが落ちて予定間隔の1.5倍を超えた場合は、
**「N時間 実行されていません」**と赤バナーで警告する。
実行が止まったこと自体に気づけるようにしてある。

**「起床」列**は、寝ているのを見つけて起こした回数。
ここが増え続けるなら実行間隔が足りていないので、cron を詰めること。
直近30回のドットも、起こした回はオレンジの下線が付く。
...
現在は **6時間おき（JST 02:17 / 08:17 / 14:17 / 20:17）**。

### なぜ6時間おきで、しかも `:17` なのか

**GitHub Actions のスケジュールは実行が欠落する。** 公式に「遅延することがある」と
明記されており、実際に落ちる。2026-08-03 の 11:00 UTC (JST 20:00) は発火しなかった。

対策は2つ。

1. **`:00` ちょうどを避ける。** 全世界のジョブが集中する最悪の時間帯。
   `:17` のような半端な分にすると欠落しにくい。
2. **回数を増やして冗長化する。** 2回中1回落ちると24時間空くが、
   4回中1回なら12時間で済む。1回の欠落が致命傷にならない。

実測では Streamlit は **13.5時間で寝ていた**ため、12時間間隔では余裕がない。
...
| 間隔 | cron | JST |
|---|---|---|
| 4時間おき | `17 3,7,11,15,19,23 * * *` | — |
| **6時間おき** | **`17 5,11,17,23 * * *`** | **02:17 / 08:17 / 14:17 / 20:17** |
| 8時間おき | `17 7,15,23 * * *` | 08:17 / 16:17 / 00:17 |
| 12時間おき | `17 11,23 * * *` | 08:17 / 20:17 |
| 1日1回 | `17 23 * * *` | 08:17 |

先頭の数字が分。**`:00` や `:30` は避けること**（欠落しやすくなる）。

パブリックリポジトリなら Actions の実行時間は無料なので、間隔を詰めても課金されない。
